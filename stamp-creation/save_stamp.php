...
try {
    // ユーザーがログインしていない場合、エラーレスポンスを返す
    if (!isset($_SESSION['user_id'])) {
        send_json_response(false, 'ユーザーがログインしていません。');
    }
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST") {
        ...
        $image_path = "/assets/images/generated_stamps/" . $filename; // "lesson-management-system" を除去
        ...
        send_json_response(true, 'スタンプが正常に作成されました。', [
            'stamp_id' => $new_stamp_id,
            'image_path' => $image_path,
            'hobby' => $hobby
        ]);
    } else {
        send_json_response(false, '不正なリクエストです。');
    }
} catch (Exception $e) {
    error_log('Error in save_stamp.php: ' . $e->getMessage());
    send_json_response(false, 'エラーが発生しました: ' . $e->getMessage());
}
